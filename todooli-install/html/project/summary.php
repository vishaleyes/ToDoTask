...
<div class="wrapper">
    ...
    <form id="selectionForm" name="selectionForm" method="post" action="">
     <div class="field sidelist">
        <label>Search:</label>
        <input type="text" id="keyword"  name="keyword" value="<?php echo $keyword;?>" class="textbox width150" />
    </div>
    <div class="field sidelist">
...
{
	$search="AND (projectName LIKE '%".$keyword."%' or issues LIKE '%".$keyword."%' or actionItems LIKE '%".$keyword."%' or owner LIKE '%".$keyword."%' or nextMilestone LIKE '%".$keyword."%')";	
}
$sqlProject="select * from project where IF(newDate Like '%0000%',originalDate,newDate) >= '".$startDate."' && IF(newDate Like '%0000%',originalDate,newDate) <= '".$endDate."' ".$search."  ORDER BY  projectName DESC";
...
for ($i = 0; $i < count($projects) ; $i++ ) { 
  ...
  echo '<tr>';
  echo '<td width="200">';
  echo '<a href="javascript:;" onclick="editRecord('.$projects[$i]['id'].')" >'.$projects[$i]['projectName'].'</a>';
  echo '</td>';
...
function editRecord(id)
	{
		$j('#formcontent').html('<a href="javascript:;" onclick=$j("#formcontent").toggle(); class="popupEditClose"><img src="images/close.png" alt="" border="0" /></a> <iframe frameborder="0"  width="1000px" height="550px" src="<?php echo BASE_PATH;?>edit.php?Action=Edit&id='+id+'"></iframe>');
		$j('#formcontent').toggle();
	}
	var $j = jQuery.noConflict();
	$j(document).ready(function()
	{
...
<div id="formcontent"  style="display:none; background:#FFF; position:absolute; left:10%; top:10%; z-index:500;"> 
</div>
</body>
</html>
